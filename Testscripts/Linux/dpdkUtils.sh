...
function Testpmd_Macfwd_To_Dest() {
	...
	if [[ ! $(printf "${dpdk_version_changed_mac_fwd}\n${dpdk_version}" | sort -V | head -n1) == "${dpdk_version}" ]]; then
		ptr_code="struct rte_ipv4_hdr *rte_ipv4_hdr;"
		dst_addr_code="rte_ipv4_hdr = rte_pktmbuf_mtod_offset(mb, struct rte_ipv4_hdr *, sizeof(struct ether_hdr)); rte_ipv4_hdr->dst_addr = rte_be_to_cpu_32(IPv4(${dst_addr}));"
	fi
